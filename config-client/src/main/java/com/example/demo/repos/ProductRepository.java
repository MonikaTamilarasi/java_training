...
public class ProductRepository implements CrudRepository<Product> {
	...
	public Product findById(int id) {
		try{return template.queryForObject("select * from moni_product where product_id=?",
	...
	public int update(Product t) {
		return template.update("update moni_product set product_name=?,price=? where product_id=?",
				new Object[] {
						t.getProductName(),t.getPrice(),t.getProductId()
				});
	}

	@Override
	public int remove(int id) {
		return template.update("delete from moni_product where product_id=?",id);
	}
